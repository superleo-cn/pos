...
select s.name AS shop_name,u.realname AS real_name,d.a_open_balance AS open_balance,d.c_cash_collected AS cash_in_drawer,d.b_expenses AS expenses,(d.c_cash_collected + d.h_card - d.e_next_open_balance) AS total_collection,d.d_daily_turnover AS daily_turnover,(d.b_expenses + d.c_cash_collected+d.h_card) AS total,d.create_date AS create_date,d.e_next_open_balance AS next_open_balance,d.h_card AS card_collected from ((pos_f1.tb_shop s join pos_f1.tb_user u) join pos_f1.tb_daily_summary d) where ((s.id = u.shop_id) and (s.id = d.shop_id) and (u.id = d.user_id))
